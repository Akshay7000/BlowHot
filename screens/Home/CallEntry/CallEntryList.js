 ...
 // getDealerList(startDate,endDate,selectedIndex)
  ...
  const getDealerList = async (start, end, index) => {
    ...
    Axios({
      ...
      .then(response => {
        console.log("responsbe",response.data.c_visit_entrySchema[response.data.c_visit_entrySchema.length-1])
  ...
  const filter = (text) => {
    ...
    const newArray = array.filter(table => (
      ((table?.Ship_party?.ACName)?.toLowerCase())?.includes(text.toLowerCase())) || ((table?.ac_cty?.call)?.toLowerCase())?.includes(text.toLowerCase())
